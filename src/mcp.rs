...
use crate::{format_device_status, set_device_brightness, set_device_power};
...
impl WledMcpServer {
    ...
    #[tool(
        description = "Get WLED device status (power, brightness, color, effect). By default, the default device is used, but you can optionally specify a device name or IP address."
    )]
    async fn wled_status(
        ...
        let device = params.device.clone();
        match tokio::task::spawn_blocking(move || {
            format_device_status(device.as_deref()).map_err(|e| e.to_string())
        })
        .await
        {
